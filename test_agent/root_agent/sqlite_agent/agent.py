...
from .mcp_client import get_sqlite_alchemy_client, get_brave_search_client
...
async def get_sqlite_agent():
    ...
    brave_search_tools, _ = await get_brave_search_client(async_exit_stack=async_exit_stack)

    model_name = "hosted_vllm/Qwen/QwQ-32B-AWQ"
    # model_name = "ollama_chat/qwq:latest"
    agent = LlmAgent(
        model=LiteLlm(
            model=model_name,
            api_base="http://0.0.0.0:3001/v1",
        ),
        name="sqlite_agent",
