...
if [ "$choice_framework" = "vllm" ]; then
    ...
    uv run python -m sglang.launch_server --model-path Qwen/QwQ-32B-AWQ --tp 2 --mem-fraction-static 0.8 --tool-call-parser qwen25 --reasoning-parser deepseek-r1 --port 3001 --host 0.0.0.0 --attention-backend flashinfer
else
